...
LOG = Path(__file__).resolve().parent / "data" / "telegram.log"


def log_tg(msg: str) -> None:
...
def start_code(text: str) -> str:
    """'/start 13e06b01' or '/start@Crazynewaibot 13e06b01' -> bind code."""
...
class TelegramBot:
    def __init__(self, token: str, handle: Callable[[str, str, str], str], is_admin, add_admin, bind_code: str, admin_ids: list[int]):
        ...
        self.bind_code = (bind_code or "").strip()
        self.admin_ids = set(admin_ids)
        self.offset = 0
        self.last_chat_id: int | None = None
        try:
            self.api("deleteWebhook", {"drop_pending_updates": False})
            log_tg("webhook deleted, long poll enabled")
        except Exception as exc:
            log_tg(f"deleteWebhook failed: {exc}")
    ...
    def send(self, chat_id: int, text: str) -> None:
        ...
        try:
            ...
            log_tg(f"send failed chat={chat_id}: {exc}")
    ...
    def poll_once(self) -> None:
        try:
            data = self.api("getUpdates", {"offset": self.offset, "timeout": 25}, timeout=40)
        except Exception as exc:
            log_tg(f"getUpdates failed: {exc}")
            time.sleep(2)
            return
        if not data.get("ok"):
            log_tg(f"getUpdates not ok: {data}")
            return
